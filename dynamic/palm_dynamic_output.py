...
def palm_dynamic_output(wrf_files, interp_files, dynamic_driver_file, times_sec,
    ...
    outfile.description = "Contains dynamic data from WRF mesoscale."
    outfile.author = "WRF4PALM dynamic driver generator"
    outfile.history = f"Created at {time.strftime('%c')}"
    outfile.source = "netCDF4 python"
    outfile.origin_lat = lat_center
    outfile.origin_lon = lon_center
    outfile.z = 0.0
    outfile.x = 0.0
    outfile.y = 0.0
    outfile.rotation_angle = 0.0
    outfile.origin_time = f"{origin_time} UTC"
    ...
    _val_z_levels_stag.long_name = "height above origin at w grid point"
    _val_z_levels_stag.axis = "Z"
    ...
    _val_z_soil_levels.long_name = "soil depth"
    _val_z_soil_levels.axis = "Z"
    _val_z_soil_levels.positive = "down"

    _val_y             = outfile.createVariable('y',"f4", ("y"),fill_value=fillvalue_float)
    _val_y[:]          = y[:]
    _val_y.units = "m"
    _val_y.long_name = "y-coordinate"
    _val_y.axis = "Y"

    _val_x             = outfile.createVariable('x',"f4", ("x"), fill_value=fillvalue_float)
    _val_x[:]          = x[:]
    _val_x.units = "m"
    _val_x.long_name = "x-coordinate"
    _val_x.axis = "X"
    ...
    if aerosol_wrfchem:
        ...
        val_dmid.setncattr('long_name', 'aerosol diameter mid-point')
        ...
        for n in range(0, idx_len):
            val_idx[n] = n+1
        val_idx.setncattr('long_name', 'aerosol composition index')
        ...
        _val_aerosol_mass_a.setncattr('units', "kg/kg")
        ...
        _val_aerosol_mass_b.setncattr('units', "kg/kg")
        if nf2a == 1.0:
    ...
    def add_interpDim(dynam_chem_variables):
        ...
        # surface pressure
        _val_surface_forcing_surface_pressure = outfile.createVariable('surface_forcing_surface_pressure', "f4",("time"), fill_value=fillvalue_float)
        _val_surface_forcing_surface_pressure.setncattr('units', "Pa")
        _val_surface_forcing_surface_pressure.setncattr('long_name', "surface pressure")
        
        # geostrophic wind
        _val_ls_forcing_ug = outfile.createVariable('ls_forcing_ug', "f4", ("time", "z"),fill_value=fillvalue_float)
        _val_ls_forcing_ug.setncattr('units', "m/s")
        _val_ls_forcing_ug.setncattr('long_name', "u wind component geostrophic")
        
        _val_ls_forcing_vg = outfile.createVariable('ls_forcing_vg', "f4", ("time", "z"),fill_value=fillvalue_float)
        _val_ls_forcing_vg.setncattr('units', "m/s")
        _val_ls_forcing_vg.setncattr('long_name', "v wind component geostrophic")
        ...
        for var in dynam_chem_variables:
            if (var == 'pt' or var == 'qv'):
                ...
                if var == 'pt':
                    _val_init_var.setncattr('units', "K")
                    _val_init_var.setncattr('long_name', "potential temperature")
                elif var == 'qv':
                    _val_init_var.setncattr('units', "kg/kg")
                    _val_init_var.setncattr('long_name', "water vapor mixing ratio")
                    
            elif (var == 'soil_m' or var == 'soil_t'):
                _val_init_var = outfile.createVariable('init_'+ var,"f4", ("zsoil", "y", "x"),fill_value=fillvalue_float)
                _val_init_var.setncattr('lod', 2)
                if var == 'soil_m':
                    _val_init_var.setncattr('units', "m^3/m^3")
                    _val_init_var.setncattr('long_name', "volumetric soil moisture")
                elif var == 'soil_t':
                    _val_init_var.setncattr('units', "K")
                    _val_init_var.setncattr('long_name', "soil temperature")
                    
            elif var == 'u':
                _val_init_var = outfile.createVariable('init_atmosphere_'+ var, "f4", ("z", "y", "xu"),fill_value=fillvalue_float)
                _val_init_var.setncattr('lod', 2)
                _val_init_var.setncattr('units', "m/s")
                _val_init_var.setncattr('long_name', "u-component of wind")
                
            elif var == 'v':
                _val_init_var = outfile.createVariable('init_atmosphere_'+ var, "f4", ("z", "yv", "x"),fill_value=fillvalue_float)
                _val_init_var.setncattr('lod', 2)
                _val_init_var.setncattr('units', "m/s")
                _val_init_var.setncattr('long_name', "v-component of wind")
                
            elif var == 'w':
                _val_init_var = outfile.createVariable('init_atmosphere_'+ var, "f4", ("zw", "y", "x"),fill_value=fillvalue_float)
                _val_init_var.setncattr('lod', 2)
                _val_init_var.setncattr('units', "m/s")
                _val_init_var.setncattr('long_name', "w-component of wind")
                ...
                if (var == 'PM10' or var == 'PM2_5_DRY'):
                    ...
                    _val_init_var.setncattr('long_name', var.upper() + ' concentration')
                else:
                ...
                if (var == 'soil_m' or var == 'soil_t'):
                    ...
                    for side in boundary:
                        if (side == 'left' or side == 'right'):
                            if var == 'u':
                                _val_ls_forcing = outfile.createVariable('ls_forcing_'+ side +'_'+ var, "f4", ("time", "z", "y"),
                                        fill_value=fillvalue_float)
                                _val_ls_forcing.setncattr('units', "m/s")
                            elif var == 'v':
                                ...
                                _val_ls_forcing = outfile.createVariable('ls_forcing_'+ side +'_'+ var, "f4", ("time", "z", "y"),
                                        fill_value=fillvalue_float)
                                if var == 'pt':
                                    _val_ls_forcing.setncattr('units', "K")
                                elif var == 'qv':
                                    _val_ls_forcing.setncattr('units', "kg/kg")
                        ...
                        if var not in atmos_var:
                            if (var == 'PM10' or var == 'PM2_5_DRY'):
                                _val_ls_forcing.setncattr('long_name', var.upper() + ' concentration')
                            else:
                                _val_ls_forcing.setncattr('long_name', var.upper() + ' concentration')
    ...
    def add_interpValues(dynam_chem_variables):
        ...
        if not nested_domain:
            ...
            for ts in range(0, len(interp_files)):
                # geostrophic wind calculation with robust error handling
                ...
                try:
                    ...
                    if np.any(np.isnan(ug)) or np.any(np.isnan(vg)):
                        print(f"Warning: NaN values in geostrophic wind at timestep {ts}. Using default values.")
                        ug = np.ones_like(z_levels) * 5.0  # Default U wind
                        vg = np.ones_like(z_levels) * 2.0  # Default V wind
                    ...
                    print(f"Error calculating geostrophic wind at timestep {ts}: {e}")
                    print("Using default geostrophic wind values")
                    # Use default values
                    ug = np.ones_like(z_levels) * 5.0
                    vg = np.ones_like(z_levels) * 2.0
                    _val_ls_forcing_ug = outfile.variables['ls_forcing_ug']
                    _val_ls_forcing_vg = outfile.variables['ls_forcing_vg']
                    _val_ls_forcing_ug[ts, :] = ug
                    _val_ls_forcing_vg[ts, :] = vg
                ...
                # Open interpolated file for this timestep
                infile = netCDF4.Dataset(interp_files[ts], "r", format="NETCDF4")
                
                # surface pressure - handle dimension naming properly
                try:
                    ...
                    # Get the actual shape of the surface pressure data
                    pressure_data = surface_forcing_surface_pressure[0]
                    if len(pressure_data.shape) == 2:  # 2D array (y, x)
                        avg_pressure = np.mean(pressure_data)
                    elif len(pressure_data.shape) == 3:  # 3D array (time, y, x)
                        avg_pressure = np.mean(pressure_data[0])
                    else:
                        avg_pressure = pressure_data  # scalar value
                    
                    _val_surface_forcing_surface_pressure[ts] = avg_pressure
                    
                except Exception as e:
                    print(f"Error processing surface pressure at timestep {ts}: {e}")
                    print("Using default surface pressure value")
                    _val_surface_forcing_surface_pressure[ts] = 101325.0  # Default sea level pressure
                
                # other dynamical & chemical variables except: soil_t, soil_m, u, v, w
                for var in dynam_chem_variables:
                    if (var == 'soil_m' or var == 'soil_t' or var == 'u' or var == 'v' or var == 'w'):
                        continue
                    else:
                        try:
                            if (var == 'h2so4' or var=='hno3' or var== 'nh3' or var== 'ocnv' or var== 'ocsv' or var == 'no' or var == 'no2' or var == 'no3' or var == 'o3'):
                                var_upper = var.upper()
                            else:
                                var_upper = var
                            
                            # Try to find the variable with proper dimension names
                            var_name = 'init_atmosphere_' + var_upper.lower()
                            if var_name not in infile.variables:
                                var_name = 'init_atmosphere_' + var_upper
                            
                            init_var = infile.variables[var_name]
                            
                            for side in boundary:
                                try:
                                    _val_ls_forcing_var = outfile.variables['ls_forcing_'+ side +'_'+var_upper]
                                    
                                    if side == 'left':
                                        boundary_data = init_var[0, :, :, 0]
                                    elif side == 'right':
                                        boundary_data = init_var[0, :, :, -1]  # Use -1 instead of dimensions['xdim'] - 1
                                    elif side == 'south':
                                        boundary_data = init_var[0, :, 0, :]
                                    elif side == 'north':
                                        boundary_data = init_var[0, :, -1, :]  # Use -1 instead of dimensions['ydim'] - 1
                                    elif side == 'top':
                                        boundary_data = init_var[0, -1, :, :]  # Use -1 instead of dimensions['zdim'] - 1
                                    
                                    _val_ls_forcing_var[ts, :, :] = boundary_data
                                    
                                except Exception as e:
                                    print(f"Error processing {var_upper} boundary {side} at timestep {ts}: {e}")
                                    continue
                                
                        except Exception as e:
                            print(f"Error processing variable {var} at timestep {ts}: {e}")
                            continue
                ...
                try:
                    ...
                    # Use array shapes instead of hardcoded dimensions
                    u_shape = init_atmosphere_u[0].shape
                    v_shape = init_atmosphere_v[0].shape
                    w_shape = init_atmosphere_w[0].shape

                    uxleft  = init_atmosphere_u[0, :, :, 0]
                    uxright = init_atmosphere_u[0, :, :, u_shape[2] - 1]
                    vysouth = init_atmosphere_v[0, :, 0, :]
                    vynorth = init_atmosphere_v[0, :, v_shape[1] - 1, :]
                    wztop   = init_atmosphere_w[0, w_shape[0] - 1, :, :]
                    ...
                    # Write boundary conditions with proper dimension handling
                    ...
                    _val_ls_forcing_u_north[ts, :, :] = init_atmosphere_u[0, :, u_shape[1] - 1, 1:]
                    _val_ls_forcing_u_top[ts, :, :] = init_atmosphere_u[0, u_shape[0] - 1, :, 1:]
                    ...
                    _val_ls_forcing_v_right[ts, :, :] = init_atmosphere_v[0, :, 1:, v_shape[2] - 1]
                    _val_ls_forcing_v_south[ts, :, :] = vysouth
                    _val_ls_forcing_v_north[ts, :, :] = vynorth
                    _val_ls_forcing_v_top[ts, :, :] = init_atmosphere_v[0, v_shape[0] - 1, 1:, :]
                    ...
                    _val_ls_forcing_w_right[ts, :, :] = init_atmosphere_w[0, :, :, w_shape[2] - 1]
                    _val_ls_forcing_w_south[ts, :, :] = init_atmosphere_w[0, :, 0, :]
                    _val_ls_forcing_w_north[ts, :, :] = init_atmosphere_w[0, :, w_shape[1] - 1, :]
                    _val_ls_forcing_w_top[ts, :, :] = wztop
                    
                except Exception as e:
                    print(f"Error in mass balancing at timestep {ts}: {e}")
